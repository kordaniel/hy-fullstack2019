...
const Menu = () => {
  return (
    <div>
      <Link style={paddingRight} to='/'>anecdotes</Link>
      <Link style={paddingRight} to='/create'>create new</Link>
      <Link style={paddingRight} to='/about'>about</Link>
    </div>
  )
}
...
const Anecdote = ({ anecdote }) => (
  <div>
    ...
    <div style={paddingBottom}>has {anecdote.votes} votes</div>
    <div style={paddingBottom}>for more info see <a href={anecdote.info}>{anecdote.info}</a></div>
  </div>
)
...
const App = () => {
  ...
  const showNotification = (message, seconds = 10) => {
